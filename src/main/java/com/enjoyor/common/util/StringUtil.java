...
public final class StringUtil { 
    ...
    public static int getSelectMonth(String time){
    	...
    	int month = 0;
    	month = Integer.parseInt(time.substring(time.indexOf("-",0)+1, time.lastIndexOf("-")));
    	return month;
    }
...
//    }
	...
	public static String getID(){
		...
		String str = String.valueOf(System.currentTimeMillis()) + String.valueOf(random1.nextInt());
		return str;
	}
	...
	public static String getUniqueString(){
		...
		java.text.SimpleDateFormat dfmoth = new java.text.SimpleDateFormat("yyyyMMddHHmmss"); 
		//String ID =dfmoth.format(Calendar.getInstance().getTime())+Double.toString(Math.random()).substring(2, 10);	
		String ID =dfmoth.format(Calendar.getInstance().getTime())+genRandomNum(8);	
		return ID;
	}
	...
	public static Map<String, String> getAllPropertiesFromResources(String filePath) {
		InputStream in=null;
    	Map<String, String> m = null;
		try {
			in = new ClassPathResource(filePath == null ? "resources.properties" : filePath).getInputStream();
			Properties p = new Properties();
			p.load(in);
     ...
     * @return  生成list	
     */ 
	public static List resolveXML(String xml){
		...
		try {
			if (xml != null && xml.compareTo("") != 0) {
				...
				Iterator it =root.elements().iterator();//ArrayOfDouble
				while(it.hasNext()){
					Element info = (Element)it.next();
					Iterator doubleIt = info.elements().iterator();//double
					strs = new String[2];
					index = 0;
					while(doubleIt.hasNext()){
						Element doubleElement =(Element) doubleIt.next(); 
						
						strs[index] = doubleElement.getText();
						index++;
					}
					list.add(strs);
				}
     ...
     * 解析xml 
     * @param  	传入xml
     * @return  生成"1,2;3,4"形式的字符串	
     */ 
	public static String resolveXMLToString(String xml){
		Document document;
		String json = null;
		try {
			if (xml != null && xml.compareTo("") != 0) {
				...
				while(it.hasNext()){
					...
					while(doubleIt.hasNext()){
						Element doubleElement =(Element) doubleIt.next(); 
						json += doubleElement.getText();
						if(doubleIt.hasNext())json += ",";
					}
					if(it.hasNext()) json+=";";
				}
			}
	...
	/** 
     * 解析html样式生成手机页面使用的样式 
     * @param  	传入FCKeditor生成的html样式文章
     * @return  string
     */
	public static String strToPhone(String str){
		 if (str == null || str.compareTo("") == 0){
	            return "";
	        }
		  String regx = "Jane Campion directed \"The Piano\" in 1993.";
	        str = str.replaceAll("style=\".*\"", ""); //去掉yang'sh
	        str = str.replace("", "；");
	        str = str.replace(",", "，");
	        str = str.replace("?", "?");
	        str = str.replace("<", "＜");
